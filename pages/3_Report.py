...
components.html(
    ...
    <button onclick="window.parent.print()" style="
        ...
        font-size: 14px;
    ">🖨️ Print Report / Save PDF</button>
    """,
    height=50
)
...
def load_logs(name,end=-1):
    ...
    if aws.is_configured():
        return aws.get_attendance_logs()
...
with tab1:
    ...
    if not redis_face_db.empty:
        ...
        # Deletion Interface
        st.write("---")
        st.subheader("Delete Registered User")
        name_list = redis_face_db['Name'].tolist()
        user_to_delete = st.selectbox('Select User to Delete', name_list)
        
        # Find role for local delete
        role_to_delete = None
        user_row = redis_face_db[redis_face_db['Name'] == user_to_delete]
        if not user_row.empty:
            role_to_delete = user_row.iloc[0]['Role']
            
        if st.button('Delete User', type='primary'):
            with st.spinner(f'Deleting {user_to_delete}...'):
                success = face_rec.delete_user_data(user_to_delete, role_to_delete)
                if success:
                    st.success(f"Successfully deleted user '{user_to_delete}' from database, S3, and Rekognition!")
                    # Refresh the page to reload database tables
                    time.sleep(1)
                    st.rerun()
                else:
